...
3. 类似onetab的功能
   ...
   - 支持在新增标签和删除标签的时候自动同步到gitee的gist，ps：基于某个墙和网络的原因，只同步到gitee，并且默认要求初始添加gitee的权限，为了更好的体验。
...
6. 第一次使用会弹窗要求一个token，这个是gitee的，为了更好的使用插件，获取gitee的token的方法：
...
9. 手动push或pull到github或者gitee的gist
...
16. 匹配百度网盘(只支持最新的外链分享，即那种生成分享链接和提取码的方式)
